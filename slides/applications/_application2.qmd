...
:::{.incremental}
+ Trouver la cause des différents *warnings* produits par les fonctions de visualisation, et les résoudre.
+ Proposer une visualisation plus pertinente de la distribution des âges.
:::
...
:::{.callout-tip collapse="true" icon=false}
## Modifications de la partie gestion des secrets

:::{.incremental}

Traiter correctement les informations confidentielles (*secrets*) :

+ Repérer le jeton d'API dans le code. Retirer le jeton d'API du code et créer à la racine du projet un fichier YAML nommé `secrets.yaml` où vous écrivez ce secret sous la forme `key: value`. 

...
:::{.incremental}
5. Dans la console, faire `renv::snapshot()` et observer le message.
...
8. Exécuter `renv::restore()` pour installer les packages nécessaires à l'exécution du projet.
9. Vérifier que le `script.R` s'exécute correctement dans ce nouvel environnement.
:::
